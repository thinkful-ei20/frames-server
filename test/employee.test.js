...
describe('Employee - /api/employee', () => {
	...
	describe('PUT /api/employee/:empoyeeId', () => {
		...
		it('should update "lastname" field for an existing employee', () => {
			let res;
			const updateItem = { 'lastname': 'Anothername', 'password' : 'password10' };
			return chai.request(app)
		...
		it('should respond with a 404 for non-existent id', function () {
			return chai.request(app)
				.put('/api/employee/notathing')
				.send({ 'lastname': 'Anothername' })
				.set('Authorization', `Bearer ${token}`)
				.catch(res => {
					expect(res).to.have.status(400);
					expect(res.response.body.message).to.equal('The employee id notathing is not valid');
				});
		});
		...
		it('should respond with 422 if email is NOT typeof string', () => {
			return chai.request(app)
				.put(`/api/employee/${employee.id}`)
				.send({'email': 123, 'password' : 'password10'})
				.set('Authorization', `Bearer ${token}`)
				.catch(res => {
		...
		it('should respond with 422 if email is NOT trimmed', () => {
			return chai.request(app)
				.put(`/api/employee/${employee.id}`)
				.send({'email': 'example@test.com ', 'password':'password10' })
				.set('Authorization', `Bearer ${token}`)
				.catch(res => {
					expect(res).to.have.status(422);
			...
			return Employee.create(anotherEmployee)
				.then(() => {
					return chai.request(app)
						.put(`/api/employee/${employee.id}`)
						.set('Authorization', `Bearer ${token}`)
						.send({ 'email': 'fake@email.com', 'password':'password10' });
				})
				.catch(res => {
					expect(res).to.have.status(400);
		...
		it('should create a new employee', () => {
			...
			return chai.request(app)
				...
				.then(_res => {
					...
					expect(res.body).to.be.an('object');
					expect(res.body).to.have.keys('availability','id', 'firstname', 'lastname', 'img', 'email', 'adminId', 'phoneNumber');
